...
public class Player {
	...
	public void firePlayerChanged() {
		PlayerListener[] listenerList = (PlayerListener[]) listeners
	...
	/**
	 * function playTurn(Board board, Bag bag) return true if the game is
	 * finished
	 */
	/*
	 * public boolean playTurn(Board board, Bag bag) { Scanner scan = new
	 * Scanner(System.in); printState(board); int wordScore = 0; boolean
	 * end=false; do {System.out.println(
	 * "What do you want to do? (x y word [h|v], help for more options)");
	 * String result = scan.nextLine();
	 * 
	 * String args[] = result.split(" ");
	 * if(args[0].equalsIgnoreCase("endgame")) {
	 * System.out.println("End of the game"); System.out.println(board);
	 * System.out.println(name+" have "+score+" points"); return true; } else
	 * if(args[0].equalsIgnoreCase("help")) {
	 * System.out.println("Adding a word on the board : x y word [h|v]");
	 * System.out.println("Changing letters : change letters");
	 * System.out.println("Ending the game : endgame"); continue; }
	 * if(args[0].equalsIgnoreCase("change")) { changeLetters(args[1], bag);
	 * end=true; } else { try { wordScore = 0; int x =
	 * Integer.parseInt(args[0])-1; int y = Integer.parseInt(args[1])-1; String
	 * word = args[2]; boolean horizontal =
	 * (args[3].equalsIgnoreCase("h")?true:false); wordScore = board.addWord(x,
	 * y, horizontal, word, rack); } catch (ArrayIndexOutOfBoundsException e) {
	 ...
	 * System.out.println("Error : "+e.getMessage()); } } if (wordScore == 0) {
	 * System.out.println("This action isn't possible."); } } while((wordScore
	 * == 0)&&!end); System.out.println("You win "+wordScore+" points"); score
	 * += wordScore; while(rack.length()<Rack.SIZE) {
	 * rack.addLetter(bag.getLetter()); } return false; }
	 */

	protected void changeLetters(String letters, Bag bag) {
		for (int i = 0; i < letters.length(); ++i) {
			if (rack.getLetter(letters.charAt(i)) == null)
				throw new IllegalArgumentException("Impossible de changer "
						+ letters.charAt(i)
	...
	/**
	 * function printState Show the state of the player, the board and his rack
	 * 
	 * @param board
	 */
	/*
